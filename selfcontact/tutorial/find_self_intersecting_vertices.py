...
def save_mesh(mesh, verts_exterior, path='mesh_out.obj'):

    verts_exterior_np = verts_exterior.cpu().numpy()
...
def main(args):
    ...
    save_mesh(mesh, verts_exterior, 
     osp.join(OUTPUT_DIR, f'{MESH_FN}_without_segment_tests.obj'))
    ...
    # The segment tests find self-intersections in crook regions and
    # ignore them.
    (verts_v2v_min, verts_incontact, verts_exterior), _ \
    = sc_module.segment_vertices(
        vertices,
